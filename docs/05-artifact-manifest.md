...
Snapshot: **24.09.2026, 17:02** (kindle `card`, live card + local `kindle-eink/` phase + n8n API dumps). Repo total: ~205 files / ~27 MB. All `.png`/`.js`/`.sh`/`.bin`/`.log` copies are bit-identical to their card origin (sizes verified at copy time) — **exception: `binaries/dash` is the T16 (24.09) Go 1.23.12 rebuild of the final source — and now ALSO the on-card binary** (deployed 24.09 13:29Z in T17; repo == card, SHA re-verified after deploy; the old mmap build is no longer on the card, kept in the repo as `dash`).

## `artifacts/` — device scripts & binaries (card `/mnt/us/`)

| File | Size | Provenance / notes |
|---|---|---|
| `refresh.sh` | 2,599 B — **on card since 24.09 13:29Z (T17 deploy; repo == card)** | **v4** (live on card since 09:26Z): 10 s tick; boot = cache restore (`/mnt/us/dashboard.png` → `/tmp`) + render; render every 30 s via `dash render` (now: T16 build's `eips -g` exec — **visible**); download every 300 s via `dash get` (first URL wins: live webhook → LAN n8n); hot re-stage `/mnt/us/dash` → `/tmp` on size change; wifi re-enable per tick; log → `/mnt/us/refresh.log`. SHA-256 `d8d84e9947476bfa51231f0980a987bdd3696446e305ce03d2f2ee554078508c` (pre-T14: `b1c06190…`) |
...
| `binaries/dash` | 5,177,496 B | **T16 (24.09): final-architecture build with Go 1.23.12** — `CGO_ENABLED=0 GOOS=linux GOARCH=arm GOARM=7 go build -trimpath -ldflags="-s"` from `src/kindle-dash/` (static, stripped, ELF 32-bit ARM EABI5; Go 1.23 kernel floor 3.0 → compatible with the Voyage's 3.0.35; `get` = fetch + decode + grayscale → single-IDAT PNG, display = `eips -g` exec, no mmap write). SHA-256 `ce74c7b44fe56c1700ced93d097ff09c993956fe1172da3f6f6ad75198f3f942`; `strings` clean (no embedded build paths — `-trimpath`, supersedes the T11 same-size patch). **Deployed on card 24.09 13:29Z (T17, with `refresh.sh`); repo == card, SHA re-verified (replaces the slot's first build 6,291,616 B, SHA-256 `1d5c8979e7a01033acee51d18fed8b3b59fb41688e6a89e0c197198d5e791a67` — sanitized copy of the I10-era mmap build). Next deploy = T20** (DASH_SCALE letterbox rebuild; `dash` + `refresh.sh` remain version-locked: deploy both) |
...
| `dashboard.png` | 792,764 B | On-card cache (mtime 06:43Z) = last download. **Measured 24.09 14:57Z: 1072×1448, 8-bit, RGB colortype 2, 25 IDAT** — "single-IDAT grayscale" (as previously stated here, in `docs/03`, and `AGENTS.md`) was a **doc error** (all corrected). Never eips-rendered pre-T16 (mmap writes invisible); eips-rendered at the T17 boot ~14:19Z (rc=0) — faithful vs. zoom-crop **[open]**. **The frame the user actually sees is the v2/v3-era eips-rendered grayscale, bistably retained.** SHA-256 `cbf48177d8aa4b86016d4f03656d784646a25250fbed5d9dab01cd4f7952f4ce` (repo == card) |
