...
Snapshot: **24.09.2026, 17:42** (kindle `card`, live card + local `kindle-eink/` phase + n8n API dumps). Repo total: ~205 files / ~27 MB. All `.png`/`.js`/`.sh`/`.bin`/`.log` copies are bit-identical to their card origin (sizes verified at copy time) — **exceptions (T20 in progress, 17:42Z):** `binaries/dash` is now the **T20** Go 1.23.12 rebuild (DASH_SCALE letterbox; **not yet on the card** — the on-card binary is still the T16 build deployed 24.09 13:29Z, SHA `ce74c7b4…`); `refresh.sh` is 2,900 B in the repo vs 2,599 B on card (T20 adds `export DASH_SCALE=0.8` + restage NOTE); the old mmap build is kept in the repo as `dash`.

## `artifacts/` — device scripts & binaries (card `/mnt/us/`)

| File | Size | Provenance / notes |
|---|---|---|
| `refresh.sh` | 2,900 B (repo, T20) — **on card: 2,599 B since 24.09 13:29Z (T17 deploy)** | **v4** (live on card since 09:26Z): 10 s tick; boot = cache restore (`/mnt/us/dashboard.png` → `/tmp`) + render; render every 30 s via `dash render` (T16 build's `eips -g` exec — **visible**); download every 300 s via `dash get` (first URL wins: live webhook → LAN n8n); hot re-stage `/mnt/us/dash` → `/tmp` on size change; wifi re-enable per tick; log → `/mnt/us/refresh.log`. **T20 additions (repo only, 17:42Z):** `export DASH_SCALE=0.8` + a NOTE comment (the T20 dash has the SAME size as T16 → the size-based hot re-stage is blind to a same-size swap; boot re-stage after reboot is the effective deploy path). SHA-256 `b7e3143837a16b853c9e27e005e659c923bc5338792d3311cfd9e89093cda50e` (repo); on-card SHA-256 `d8d84e9947476bfa51231f0980a987bdd3696446e305ce03d2f2ee554078508c` (pre-T14: `b1c06190…`) |
